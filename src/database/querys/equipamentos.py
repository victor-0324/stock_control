...
class EquipamentosQuerys:
    ...
    def add_novo(cls, connection, modelo, nome, data):
        ...
        criar_novo_equipamento = Equipamentos(
            modelo=modelo.upper(), estado="Usando", data=data, cliente=nome
        )
    ...
    def deletar(cls, connection, equipamento_id):
        ...
        equipamento = (
            connection.session.query(Equipamentos).filter_by(id=equipamento_id).first()
        )
    ...
    def update_trocar(cls, connection, arg1, arg2):
        """someting"""
        query = connection.session.query(Equipamentos).filter_by(modelo=arg1).first()
